...
unsigned int getSeedValue3d(char *key, int l, unsigned int seed)
{
	int i;
	for (i = 0; i < 32; i++)
...
int getDim3d(unsigned int seed)
{
...
void genHash3d(char *key, unsigned int seed, int bit_size, char *hash_value)
{
	int s, t, r, c, w, X, Y, Z;
	...
	seed = (unsigned int)getSeedValue3d(key, l, seed);
	seed = tab[seed % 2293];
	seed = getSeedValue3d(key, l, seed);
	r = getDim3d(seed);
	while (1)
	{
		seed = (unsigned int)getSeedValue3d(key, l, seed);
	...
	for (i = 0; i < X; i++)
	{
		for (j = 0; j < Y; j++)
		{
			for (k = 0; k < Z; k++)
			{
				seed = (unsigned int)getSeedValue3d(key, l, seed); // hash function calls for seed value.
				hv = (unsigned int)murmur3(key, l, seed);					 // hash function calls for a single bit value.
				pos = hv % 31;
				bit = (hv & (1 << pos)) >> pos;
				vector[i][j][k] = bit;
				seed = hv;
	...
	{
		seed = (unsigned int)getSeedValue3d(key, l, seed);
		hv = (unsigned int)murmur3(key, l, seed);
		i = hv % X;
		seed = hv;
		seed = (unsigned int)getSeedValue3d(key, l, seed);
		hv = (unsigned int)murmur3(key, l, seed);
